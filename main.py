...
# ==== Scrapers (mantén estos módulos con las firmas indicadas) ====
...
# ==== Normalización + Lista blanca ====
from normalizer import normalize_row
from whitelist import is_allowed


# ---------------------------------------------------------
# Utilidades de navegación: aceptar cookies + carga suave
# ---------------------------------------------------------
...
async def run():
    ...
    async with async_playwright() as p:
        ...
        # User-Agent “normal” para evitar variantes ultra-minimalistas
    ...
    # ---------------------------------------------------------
    # Normalización (Marca, Descuento, Días, Categoria_norm, Link_show, Fuente_corta)
    # ---------------------------------------------------------
    rows = [normalize_row(r) for r in rows]

    # ---------------------------------------------------------
    # Lista blanca de dominios: deja SOLO lo que venga de las
    # fuentes oficiales que definiste (whitelist.is_allowed)
    # ---------------------------------------------------------
    clean = []
    for r in rows:
        link = r.get("Link_show") or r.get("Link") or r.get("Fuente")
        if link and is_allowed(link):
            clean.append(r)
    rows = clean
    print(f"[INFO] Total filas tras whitelist: {len(rows)}")

    # ---------------------------------------------------------
    # Resumen por proveedor (log)
    # ---------------------------------------------------------
    prov_counter = Counter([r.get("Proveedor", "—") for r in rows])
    for prov, count in prov_counter.items():
        print(f"[INFO] {prov}: {count} filas")
